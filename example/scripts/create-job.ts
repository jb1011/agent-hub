...
async function providerPriceFromChain(escrowAddress: string, registryProviderId: string): Promise<bigint> {
  ...
  const registered = await registry.getProvider(registryProviderId) as { price: bigint };
...
if (process.env.SIGNER_WALLET_PK?.trim()) {
  await ensurePaymentAllowance(job.user_wallet, transaction.to, job.provider_id);
} else {
